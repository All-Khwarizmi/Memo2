  ...
  return (
    <main className="h-screen w-screen">
      ...
      <div className="flex h-full w-full flex-col items-center gap-5">
        <div className="border-black-500 h-[40%] max-h-[40%] w-[40%] overflow-scroll rounded border-2 border-solid bg-gray-200">
          <div className="p-2 text-black">Mot recherché: </div>
          <div className="text-black">
            {translations?.map((trad: any, index) => {
              return (
                <div key={index}> dangerouslySetInnerHTML={__html: trad.target}</div>
              );
            })}
          </div>
        </div>
